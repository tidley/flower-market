...
import type { ChallengeInput } from './types.ts';
import type { ChallengeExecutionInput, ProofRevealInput } from './pipeline.ts';
...
describe('v0.1 conformance fixtures', () => {
  it('proof vectors match expected validity', () => {
    for (const v of proofVectors.vectors) {
      expect(verifyMerkleProof(v.leafHash, v.proof as MerkleProofNode[], v.expectedRoot), v.name).toBe(v.expected);
    }
  });

  it('settlement vectors produce expected order/totals/exclusions', () => {
    const out = settleChallenge(settlementVectors.input as ChallengeInput, settlementVectors.reveals);
  ...
  it('pipeline vectors enforce deadline/proof/root invalidation', () => {
    const out = settleChallengeFromProofs(
      pipelineVectors.challenge as ChallengeExecutionInput,
      pipelineVectors.reveals as ProofRevealInput[],
    );
